...
const unformatTimestamp = (formattedTimestamp) => {
  const timeComponents = formattedTimestamp
    .split(':')
...
const getTimestamps = (videos) => videos.map((video) => {
...
const isNewDesign = () => {
  ...
  const newDesign = newDesignAnchor && oldDesignAnchor.getAttribute('hidden') !== null;
...
const displayLoader = () => {
  const playlistSummary = document.querySelector(
    isNewDesign() ? '#ytpdc-playlist-summary-new' : '#ytpdc-playlist-summary-old',
  );
...
const addSummaryToPage = (summary) => {
  ...
  const metadataSection = document.querySelector(
    newDesign ? config.metadataContainer.main : config.metadataContainer.fallback,
  );
...
const setupPlaylistObserver = (start) => {
...
const setupEventListeners = (start) => {
